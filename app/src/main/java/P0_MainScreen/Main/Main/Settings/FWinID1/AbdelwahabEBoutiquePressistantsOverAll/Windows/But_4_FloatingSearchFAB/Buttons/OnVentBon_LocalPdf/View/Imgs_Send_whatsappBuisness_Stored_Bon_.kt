...
fun Imgs_Send_whatsappBuisness_Stored_Bon_(
    ...
    // ── Derive the baseName used by convertAllPdfPagesToJpgs ──────────────────
    // Must match exactly the naming in initiateBackgroundPdfCreation_NewP.
    ...
    // ── Query MediaStore for saved JPG URIs matching this bon ─────────────────
    val jpgUris by produceState<List<Uri>>(
    ...
    Row(
        ...
        FloatingActionButton(
            ...
            containerColor = when {
                ...
                else                 -> Color(0xFF1E88E5)   // blue — distinct from PDF (orange) and WA PDF (green)
            }
        ) {
            Icon(
                imageVector = when {
                ...
                contentDescription = "Envoyer images via WhatsApp Business",
                tint = Color.White
            )
        }
...
// ─────────────────────────────────────────────────────────────────────────────
// MediaStore query — find all JPGs saved by convertAllPdfPagesToJpgs
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Returns URIs for every JPG whose display-name starts with [baseName] and
 * whose RELATIVE_PATH sits inside `Downloads/BonsWhatsApp/`.
 *
 * Covers both naming conventions used by [convertAllPdfPagesToJpgs]:
 *  - single page  → `{baseName}.jpg`
 *  - multi-page   → `{baseName}_p1.jpg`, `{baseName}_p2.jpg`, …
 *
...
private fun findBonJpgsFromMediaStore(context: Context, baseName: String): List<Uri> {
    ...
    return try {
        ...
        // LIKE match: covers both "baseName.jpg" and "baseName_p*.jpg"
        val selection   = "${MediaStore.Downloads.DISPLAY_NAME} LIKE ? " +
                "AND ${MediaStore.Downloads.RELATIVE_PATH} LIKE ?"
        val selectionArgs = arrayOf("$baseName%.jpg", "%BonsWhatsApp%")

        context.contentResolver
            .query(collection, projection, selection, selectionArgs, "${MediaStore.Downloads.DISPLAY_NAME} ASC")
            ?.use { cursor ->
                buildList {
...
// ─────────────────────────────────────────────────────────────────────────────
// WhatsApp Business — send images
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Opens WhatsApp Business with one or more JPEG images attached.
 * Uses ACTION_SEND for a single image and ACTION_SEND_MULTIPLE for several.
 * MediaStore URIs are already world-readable — no FileProvider needed.
 */
