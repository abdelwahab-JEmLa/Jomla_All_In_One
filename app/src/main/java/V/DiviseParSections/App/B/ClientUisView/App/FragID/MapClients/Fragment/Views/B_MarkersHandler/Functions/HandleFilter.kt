...
fun filterClientsBasedOnMode(
    ...
    return when (currentFilterMode) {
        ...
        MapClientsViewModel.VisibleClientsNow.showClientsWithConfirmedProducts -> {
            val clientsWithConfirmedProducts =
                viewModel.groupeRepositorysProtoAvJuin3.repositorys_Model
                    ...
                    .filter { bonAchat ->
                        bonAchat.etateActuellementEst == TransactionVent.EtateActuellementEst.A_COMMANDE_CONFIRME
                                || bonAchat.etateActuellementEst == TransactionVent.EtateActuellementEst.ON_MODE_COMMEND_ACTUELLEMENT
                    }
        ...
        MapClientsViewModel.VisibleClientsNow.AFFICHE_CIBLE_POUR_VENDEUR -> {
            clientDataBaseSnapList.filter {
                viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.Cible
                        || viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.ON_MODE_COMMEND_ACTUELLEMENT
        ...
        MapClientsViewModel.VisibleClientsNow.CIBLE_ET_CELUIT_ON_A_PASSE_A_EUX -> {
            clientDataBaseSnapList.filter {
                viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.Cible
                        || viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.ON_MODE_COMMEND_ACTUELLEMENT
                        || viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.A_EVITE
                        || viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.AVEC_MARCHANDISE
                        || viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.A_COMMANDE_CONFIRME
                        || viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.ACHETEUR_NON_DISPO
                        || viewModel.getLastTransaction(it)?.etateActuellementEst == TransactionVent.EtateActuellementEst.FERME
            }
        }
