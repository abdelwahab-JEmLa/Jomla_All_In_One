...
@Composable
private fun BenificeAdjustmentButtons(
    allTariffsGroupedAndSorted: SortedMap<M13TarificationInfos.TypeChoisi, List<M13TarificationInfos>>,
    relative_Produit: ArticlesBasesStatsTable,
    relative_Tariff: M13TarificationInfos,
    ...
    // Benefit display and increase button
    ElevatedCard(
        onClick = {
...
@Composable
fun BenificeAjustementPrixCard(
    ...
    val benefitColor = when {
        benefice < 0 -> Color.Red
        benefice < 20 -> Color(0xFFFF9800)
        benefice < 50 -> Color(0xFF4CAF50)
        else -> Color(0xFF2196F3)
    }

    // Compact card displaying benefit details
    ElevatedCard(
        modifier = modifier.width(85.dp),
        colors = CardDefaults.elevatedCardColors(
            containerColor = benefitColor.copy(alpha = 0.1f)
        )
    ) {
        Column(
            ...
            // Benefit percentage
            val benefitPercentage = if (prixAchat > 0) {
                (benefice / prixAchat) * 100
            } else 0.0

            Text(
                text = "${String.format("%.0f", benefitPercentage)}%",
                style = MaterialTheme.typography.labelSmall,
                color = benefitColor.copy(alpha = 0.8f),
                fontSize = 9.sp
            )

            // Compact breakdown formula
            Text(
                text = "${String.format("%.0f", prixAchat)}+${String.format("%.0f", benefice)}",
                style = MaterialTheme.typography.labelSmall,
