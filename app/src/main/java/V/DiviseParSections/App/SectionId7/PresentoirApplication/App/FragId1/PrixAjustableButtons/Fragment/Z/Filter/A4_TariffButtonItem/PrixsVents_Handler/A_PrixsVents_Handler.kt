...
fun PrixsVents_Handler(
    ...
    fun handelClick() { executeClickLogic() }
...
// ── BenefitDisplayRow : sans boutons +/- ────────────────────────────────────
@Composable
fun BenefitDisplayRow(
    ...
    LaunchedEffect(isEditingTotalBenefit) {
        if (isEditingTotalBenefit) totalBenefitFocusRequester.requestFocus()
    }

    fun updateBenefitImmediately(newBenefit: Double) {
        onPriceChange(prixAchat + newBenefit, false)
    }

    fun handleTotalBenefitEditDone() {
        totalBenefitText.toDoubleOrNull()
            ?.takeIf { it >= 0 }
            ?.let { updateBenefitImmediately(it) }
    ...
    Column(
            ...
            if (isEditingTotalBenefit) {
                ...
                Text(
                    ...
                    modifier = Modifier
                        ...
                        .clickable { isEditingTotalBenefit = true; totalBenefitText = "" },
                    color = Color.White,
                    fontSize = 10.sp
                )
            }
        ...
        // ── Bénéfice unitaire (lecture seule) ────────────────────────────
        Text(
