...
object Initializer_App4 {
    suspend fun initializeAllRepositories(
        ...
        mutex.withLock {
            ...
            ).forEach { progress[it] = 0f }
        }

        val isOnline = isInternetAvailable(context)
        suspend fun seedRepo(name: String, block: suspend () -> Unit) {
            if (!isOnline) {
                markComplete(name); return
            }
            try {
                setProgress(name, 0.2f); block(); markComplete(name)
            } catch (_: Exception) {
                markComplete(name)
            }
        }
        ...
        coroutineScope {
            launch(Dispatchers.IO) {
                seedRepo("M1Produit") {
                    ...
                    M01Produit.ref.get()
                        .await().children.mapNotNull { it.getValue(M01Produit::class.java) }
                ...
                seedRepo("M3CouleurProduitInfos") {
                    if (dao_M03CouleurProduitInfos.getAll().isEmpty()) {
                        fetchWithRetry(M3CouleurProduitInfos.refFirestore)
                            ...
                            ?.let { dao_M03CouleurProduitInfos.insertAll(it) }
                            ?: return@seedRepo
                    }
            ...
            launch(Dispatchers.IO) {
                seedRepo("M13TarificationInfos") {
                    ...
                    M13TarificationInfos.ref.get().await().children.mapNotNull {
                        it.getValue(
                            M13TarificationInfos::class.java
                        )
                    }
            ...
            launch(Dispatchers.IO) {
                seedRepo("M14VentPeriode") {
                    ...
                    M14VentPeriode.ref.get().await().children.mapNotNull {
                        it.getValue(
                            M14VentPeriode::class.java
                        )
                    }
            ...
            launch(Dispatchers.IO) {
                seedRepo("M8BonVent") {
                    ...
                    M8BonVent.ref.get()
                        .await().children.mapNotNull { it.getValue(M8BonVent::class.java) }
            ...
            launch(Dispatchers.IO) {
                seedRepo("M10OperationVentCouleur") {
                    ...
                    M10OperationVentCouleur.ref.get().await().children.mapNotNull {
                        it.getValue(
                            M10OperationVentCouleur::class.java
                        )
                    }
                        .takeIf { it.isNotEmpty() }
                        ?.let { dao_M10OperationVentCouleur.insertAll(it) }
                }
            }
            launch(Dispatchers.IO) {
                seedRepo("Z_AppCompt") {
                    ...
                    Z_AppCompt.ref.get()
                        .await().children.mapNotNull { it.getValue(Z_AppCompt::class.java) }
