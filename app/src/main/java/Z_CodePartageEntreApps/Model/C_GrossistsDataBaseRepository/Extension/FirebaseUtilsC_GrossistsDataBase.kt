...
/**
 * Utility class for Firebase operations related to C_GrossistsDataBase
 */
object FirebaseUtilsC_GrossistsDataBase {
    ...
    /**
     * Start database listener for C_GrossistsDataBase
     */
    fun startDatabaseListener(
        repository: C_GrossistsDataBaseRepositoryImpl,
        onValueEventListenerCreated: (ValueEventListener) -> Unit
    ) {
        createValueEventListener(repository)?.let {
            C_GrossistsDataBaseRepository.caReference.addValueEventListener(it)
     ...
     */
    private fun createValueEventListener(repository: C_GrossistsDataBaseRepositoryImpl): ValueEventListener {
     ...
     */
     fun onDataBaseChangeListnerAndLoad(repository: C_GrossistsDataBaseRepositoryImpl): Pair<List<C_GrossistsDataBase>, Flow<Float>> {
     ...
     */
    fun checkConnectivityAndSync(repository: C_GrossistsDataBaseRepositoryImpl) {
        val currentTime = System.currentTimeMillis()
        ...
        if (isConnected != lastConnectivityState) {
            ...
            if (isConnected) {
                ...
                C_GrossistsDataBaseRepository.caReference.keepSynced(false)
