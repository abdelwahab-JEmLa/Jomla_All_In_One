...
import Z_CodePartageEntreApps.Model.A_ProduitModelNewProto.Repository.A_ProduitModelNewProtoRepository
import Z_CodePartageEntreApps.Model.A_ProduitModelNewProto.Repository.A_ProduitModelNewProtoRepositoryImpl
...
/**
 * Utility class for Firebase operations related to A_ProduitModel
 */
object FirebaseUtilsA_ProduitModel {
    ...
    fun initializeFirebaseOfflineCapability() {
        try {
            A_ProduitModelNewProtoRepository.caReference.keepSynced(true)
        } catch (e: Exception) {
            // Silent catch
        }
    }
    ...
    /**
     * Start database listener for A_ProduitModel
     */
    fun startDatabaseListener(
        repository: A_ProduitModelNewProtoRepositoryImpl,
        onValueEventListenerCreated: (ValueEventListener) -> Unit
    ) {
        createValueEventListener(repository)?.let {
            A_ProduitModelNewProtoRepository.caReference.addValueEventListener(it)
     ...
     */
    private fun createValueEventListener(repository: A_ProduitModelNewProtoRepositoryImpl): ValueEventListener {
     ...
     */
    fun onDataBaseChangeListnerAndLoad(repository: A_ProduitModelNewProtoRepositoryImpl): Pair<List<A_ProduitModel>, Flow<Float>> {
     ...
     */
    fun checkConnectivityAndSync(repository: A_ProduitModelNewProtoRepositoryImpl) {
        val currentTime = System.currentTimeMillis()
        ...
        if (isConnected != lastConnectivityState) {
            ...
            if (isConnected) {
                ...
                A_ProduitModelNewProtoRepository.caReference.keepSynced(false)
                A_ProduitModelNewProtoRepository.caReference.keepSynced(true)
