...
import Z_CodePartageEntreApps.DataBase.Z_AppComptRepository.Base.AvantJuin3._1_5_Vendeur.Proto.Extension.Log._1_5_VendeurRepositoryLogOperationsExtention
import Z_CodePartageEntreApps.DataBase.Z_AppComptRepository.Base.AvantJuin3._1_5_Vendeur.Proto.Extension.Update._1_5_VendeurRepositoryUpdatesOperaionsExtention
...
class _1_5_VendeurRepositoryImpl(
    ...
    private val updatesOperations = _1_5_VendeurRepositoryUpdatesOperaionsExtention(this)
    private val logOperations = _1_5_VendeurRepositoryLogOperationsExtention(this)
    ...
    private suspend fun loadDepuitRoom() {
        try {
            ...
            withContext(Dispatchers.IO) {
                val dataList = try {
                    appDatabase._1_5_VendeurDao().getAll()
                } catch (e: Exception) {
    ...
    private suspend fun checkDataConsistency() {
        try {
            val roomCount = withContext(Dispatchers.IO) {
                try {
                    appDatabase._1_5_VendeurDao().getCount()
                } catch (e: Exception) {
            ...
            val firebaseSnapshot = try {
                withContext(Dispatchers.IO) {
                    val task = _1_5_Vendeur_Repository.sonDataBaseRef.get()
                    Tasks.await(task)
                }
            } catch (e: Exception) {
    ...
    private fun FireBaseOnDataChangeListner() {
        synchronized(flowListenerLock) {
            ...
            if (!isFlowListenerActive.get()) {
                        ...
                        try {
                            ...
                            repositoryScope.launch(Dispatchers.IO) {
                                try {
                                    ...
                                    appDatabase._1_5_VendeurDao().insertAll(updatedList)
                                } catch (e: Exception) {
                                    Log.e(
                                        TAG,
                ...
                _1_5_Vendeur_Repository.sonDataBaseRef.addValueEventListener(flowValueEventListener!!)
    ...
    private fun removeFlowDataListener() {
        synchronized(flowListenerLock) {
            if (isFlowListenerActive.get() && flowValueEventListener != null) {
                try {
                    _1_5_Vendeur_Repository.sonDataBaseRef.removeEventListener(flowValueEventListener!!)
                } catch (e: Exception) {
        ...
        try {
            ...
            viewModelScope.launch(Dispatchers.IO) {
                try {
                    ...
                    if (dataList.isNotEmpty()) {
                        try {
                            appDatabase._1_5_VendeurDao().insertAll(dataList)
    ...
    private fun removeDataChangeListener() {
        synchronized(listenerLock) {
            if (isListenerActive.get() && valueEventListener != null) {
                try {
                    _1_5_Vendeur_Repository.sonDataBaseRef.removeEventListener(valueEventListener!!)
                } catch (e: Exception) {
