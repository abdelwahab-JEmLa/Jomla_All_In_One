...
import Z_CodePartageEntreApps.DataBase.Juin3.Proto.B_ClientInfosProtoJuin3.Repository.Z.Archive.Proto.D.Repository.Extension.FirebaseUtilsB_ClientsDataBaseNewProto
...
class B_ClientsDataBaseRepositoryImpl(
    ...
    override var modelDatas: SnapshotStateList<B_ClientsDataBaseProtoD> = mutableStateListOf()
    ...
    override fun updateUnSeulData(data: B_ClientsDataBaseProtoD?) {
        if (data == null) return
    ...
    override suspend fun onDataBaseChangeListnerAndLoad(): Pair<List<B_ClientsDataBaseProtoD>, Flow<Float>> {
        checkConnectivityAndSync()
        return Pair(modelDatas.toList(), progressRepo)
    }

    private fun firebaseUpdateData(data: B_ClientsDataBaseProtoD) {
        try {
    ...
    override suspend fun updateMultiDatas(datas: SnapshotStateList<B_ClientsDataBaseProtoD>) {
