...
class B_ClientDataBaseRepositoryImpl(
    ...
    override var modelDatas: SnapshotStateList<B_ClientDataBaseProtoC> = mutableStateListOf()
                ...
                valueEventListener = object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        for (dataSnapshot in snapshot.children) {
                            try {
                                val clientData = dataSnapshot.getValue(B_ClientDataBaseProtoC::class.java)
    ...
    private fun hasRelevantChanges(oldData: B_ClientDataBaseProtoC, newData: B_ClientDataBaseProtoC): Boolean {
    ...
    override fun deleteUnSeulData(data: B_ClientDataBaseProtoC) {
        try {
            repositoryScope.launch(Dispatchers.Main) {
        ...
        try {
            ...
            viewModelScope.launch(Dispatchers.IO) {
                try {
                    ...
                    val clientsList = mutableListOf<B_ClientDataBaseProtoC>()
    ...
    override fun addData(data: B_ClientDataBaseProtoC) {
        try {
            repositoryScope.launch(Dispatchers.Main) {
                modelDatas.add(data)
            }
    ...
    override fun updateUnSeulData(data: B_ClientDataBaseProtoC?) {
        if (data == null) {
            return
        }
    ...
    private suspend fun firebaseUpdateData(data: B_ClientDataBaseProtoC) {
        try {
            B_ClientDataBaseRepository.caReference.child(data.id.toString()).setValue(data).await()
        } catch (e: Exception) {
            // Log error
    ...
    override suspend fun updateMultiDatas(datas: SnapshotStateList<B_ClientDataBaseProtoC>) {
