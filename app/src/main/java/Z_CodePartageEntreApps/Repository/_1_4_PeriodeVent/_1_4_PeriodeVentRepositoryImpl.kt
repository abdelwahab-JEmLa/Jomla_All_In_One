    ...
    private suspend fun checkDataConsistency() {
        try {
            ...
            val firebaseSnapshot = try {
                withContext(Dispatchers.IO) {
                    val task = _1_4_PeriodeVentRepository.sonDataBaseRef.get()
                    Tasks.await(task)
                }
            } catch (e: Exception) {
    ...
    private fun FireBaseOnDataChangeListner() {
        synchronized(flowListenerLock) {
            ...
            if (!isFlowListenerActive.get()) {
                ...
                _1_4_PeriodeVentRepository.sonDataBaseRef.addValueEventListener(flowValueEventListener!!)
    ...
    private fun removeFlowDataListener() {
        synchronized(flowListenerLock) {
            if (isFlowListenerActive.get() && flowValueEventListener != null) {
                try {
                    _1_4_PeriodeVentRepository.sonDataBaseRef.removeEventListener(flowValueEventListener!!)
                } catch (e: Exception) {
    ...
    private fun removeDataChangeListener() {
        synchronized(listenerLock) {
            if (isListenerActive.get() && valueEventListener != null) {
                try {
                    _1_4_PeriodeVentRepository.sonDataBaseRef.removeEventListener(valueEventListener!!)
                } catch (e: Exception) {
