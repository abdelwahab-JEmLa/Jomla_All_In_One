...
class GroupeRepositorysProtoAvJuin3Impl(
    ...
    override fun <T> deleteData(
        ...
        try {
            repositoryScope.launch(Dispatchers.IO) {
                try {
                    when (data) {
                        is TransactionCommercial -> processDeleteOperation(
                            data = data,
    ...
    private suspend inline fun <reified T> processDeleteOperation(
        ...
        try {
            ...
            when (databaseDao) {
                is _1_3_TransactionCommercialDao -> databaseDao.delete(data as TransactionCommercial)
    ...
    override fun <DataBase> upsertUneDataEtReturnVID(
        ...
        try {
            repositoryScope.launch(Dispatchers.IO) {
                try {
                    when (data) {
                        is TransactionCommercial -> processUpsertOperation(
                            data = data,
                            databaseDao = appDatabase._1_3_TransactionCommercialDao(),
                            snapshotList = repo_1_3_TransactionCommercial.modelDatasSnapList,
                            databaseRef = C3TransactionCommercialRepository.sonDataBaseRef,
                            ...
                            onSuccess = { resultVid ->
                                Log.d(
                                    TAG,
                                    "Upsert completed for TransactionCommercial with VID: $resultVid"
                                )
                                if (resultVid <= 0) {
                                    Log.e(
                                        TAG,
    ...
    private suspend inline fun <reified DataBase> processUpsertOperation(
        ...
        val dataToUpsert = when (data) {
            is TransactionCommercial -> data.copy() as DataBase
            ...
            currentVid > 0 -> {
                ...
                when (databaseDao) {
                    is _1_3_TransactionCommercialDao -> {
                        val result =
                            databaseDao.insertAvecRetureNewVid(dataToUpsert as TransactionCommercial)
                        Log.d(TAG, "Update result for TransactionCommercial: $result")
                        result
                    }
                ...
                val newVid = when (databaseDao) {
                    is _1_3_TransactionCommercialDao -> {
                        val result =
                            databaseDao.insertAvecRetureNewVid(dataToUpsert as TransactionCommercial)
                        Log.d(TAG, "New VID for TransactionCommercial: $result")
                        result
                    }
