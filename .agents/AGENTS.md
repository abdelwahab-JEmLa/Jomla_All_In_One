...
- **Priorité absolue de `v_u`** : Dès le scan initial d'un skill `t_`, si N'IMPORTE QUEL `TODO` trouvé contient le motif `v_u` (ou `v_u_`) — qu'il s'agisse d'un TODO standard, relatif ou différé —, l'agent **ne doit résoudre ou supprimer AUCUN TODO** et doit immédiatement basculer vers l'exécution directe du skill `v_u` (copie des fichiers modifiés non commités dans le presse-papiers et `hist_copie.md`).
...
- When calling PowerShell inline commands via `run_command` (e.g. `pwsh -Command "..."`), always escape `$` signs in variable names (e.g. `\$fileCollection`) or execute dedicated python/powershell script files (like `fast_cc.py`) to prevent shell variable substitution syntax errors.
